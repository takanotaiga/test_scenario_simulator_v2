  ...
  const bool include_opposite_direction) const -> lanelet::Ids
{
  if (include_opposite_direction) {
    throw common::Error("HdMapUtils::getLeftLaneletIds with include_opposite_direction=true is not implemented yet.");
  } else {
    /// @note lefts() returns routable left adjacent lanelets in same direction.
    return getLaneletIds(routing_graphs_->routing_graph(type)->lefts(
...
{
  if (include_opposite_direction) {
    throw common::Error("HdMapUtils::getRightLaneletIds with include_opposite_direction=true is not implemented yet.");
  } else {
    /// @note rights() returns routable right adjacent lanelets in same direction.
    return getLaneletIds(routing_graphs_->routing_graph(type)->rights(
      lanelet_map_ptr_->laneletLayer.get(lanelet_id)));
  }
}
